...
function s(e) {
	...
	e.Map = e.Class(e.Events, {
		initialize: function(a, b) {
			...
			this._options = d;
			g = parseInt(b.zoom) || 4;
			d.maxZoom = Math.min(parseInt(b.maxZoom) || 22, 22);
...
(function(){IMAP&&IMAP.Marker&&IMAP.Label&&(IMAP.Marker.prototype.type="marker",IMAP.Label.prototype.type="marker");IMAP&&IMAP.Circle&&(IMAP.Circle.prototype.type="cicle");IMAP&&IMAP.Polyline&&(IMAP.Polyline.prototype.type="polyline");IMAP&&IMAP.Polygon&&(IMAP.Polygon.prototype.type="polygon");IMAP.Map.prototype.setFitView=function(c){var d,e,f,g;function h(a){try{d=a.lng<d?a.lng:d,e=a.lat<e?a.lat:e,f=a.lng>f?a.lng:f,g=a.lat>g?a.lat:g}catch(b){console.log("set fit view compare")}}c=c?c:this.getOverlayLayer().getOverlays();;d=180;e=90;g=f=0;var m=function(a,b,c){return{lng:a.lng+360*Math.asin(Math.sin(b/12756274)/Math.cos(a.lat*Math.PI/180))/Math.PI,lat:a.lat+360*Math.asin(c/12756274)/Math.PI}},k;for(k in c){var a=c[k];if(!a)return!1;if("marker"==a.type)h(a.getPosition());else if("circle"==a.type){var b=a._center,l=a._radius,a=m(b,-l,-l),b=m(b,l,l);h(a);h(b)}else if((b=a._path)&&0<b.length)for(a=0;a<b.length;a++)h(b[a])}k=new IMAP.LngLatBounds(new IMAP.LngLat(d,e),new IMAP.LngLat(f,g));this.setBounds(k)}})();
